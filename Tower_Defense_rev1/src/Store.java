...
public class Store {
	...
	public static int[] buttonID = {Values.airTowerLaser,Values.airAir,Values.airAir,Values.airAir,Values.airAir,Values.airAir,Values.airAir,Values.airTrashCan};
		...
		if(mouseButton == 1) {
			...
			if(button[i].contains(Screen.mse)) {
			  if(buttonID[i] != Values.airAir) {
			    if(buttonID[i] == Values.airTrashCan) {	
				  holdsItem = false;
		         }else{
				    heldID = buttonID[i];
				    realID =i;
						...
						for(int y=0;y<Screen.room.block.length;y++) {
							for(int x=0;x<Screen.room.block[0].length;x++) {
								if(Screen.room.block[y][x].contains(Screen.mse)) {
									if(Screen.room.block[y][x].groundID != Values.groundRoad && Screen.room.block[y][x].airID == Values.airAir) {
	...
	public void draw(Graphics g) {
		for(int i=0; i<button.length; i++) {
			...
			if(buttonID[i] != Values.airAir) g.drawImage(Screen.tileset_res[buttonID[i]], button[i].x + itemIn, button[i].y + itemIn, button[i].width - (itemIn*2), button[i].height - (itemIn*2), null);
			if(buttonPrice[i] > 0) {
	...
	if (holdsItem) {
		g.drawImage(Screen.tileset_air[heldID],Screen.mse.x - ((button[0].width - (itemIn*2) ) /2) + itemIn, Screen.mse.y - ((button[0].width - (itemIn*2) ) /2), button[0].width - (itemIn*2), button[0].height - (itemIn*2), null);
	}
}
}